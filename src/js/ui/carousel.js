...
function initMobileTeamCarousel(carousel, carouselInner, prevButton, nextButton) {
  ...
  carouselInner.className = 'carousel-inner-mobile flex overflow-x-auto snap-x snap-mandatory scroll-smooth';
  ...
  if (prevButton) {
    prevButton.className = 'carousel-control carousel-control-prev absolute top-1/2 left-4 transform -translate-y-1/2 bg-terracotta hover:bg-terracotta-light rounded-full p-2 shadow-lg z-30 flex items-center justify-center w-10 h-10';
    prevButton.style.display = 'flex';
  }
  
  if (nextButton) {
    nextButton.className = 'carousel-control carousel-control-next absolute top-1/2 right-4 transform -translate-y-1/2 bg-terracotta hover:bg-terracotta-light rounded-full p-2 shadow-lg z-30 flex items-center justify-center w-10 h-10';
  ...
  const updateCurrentIndex = () => {
    ...
    if (nextButton) {
      nextButton.style.opacity = currentIndex < allTeamMembers.length - 1 ? '1' : '0.5';
    }
  };
  
  // Scroll event listener
  const debouncedUpdate = debounce(updateCurrentIndex, 50);
  addEventListenerWithCleanup(carousel, carouselInner, 'scroll', debouncedUpdate);
  ...
  addEventListenerWithCleanup(carousel, carouselInner, 'touchstart', handleTouchStart);
  addEventListenerWithCleanup(carousel, carouselInner, 'touchmove', handleTouchMove);
  addEventListenerWithCleanup(carousel, carouselInner, 'touchend', handleTouchEnd);
...
function initDesktopTeamCarousel(carousel, carouselInner, prevButton, nextButton) {
  ...
  carouselInner.className = 'carousel-inner flex transition-transform duration-500 ease-in-out';
  
  // Reset button styles for desktop
  if (prevButton) {
    prevButton.className = 'carousel-control carousel-control-prev absolute top-1/2 left-4 transform -translate-y-1/2 bg-terracotta hover:bg-terracotta-light rounded-full p-2 shadow-lg z-10';
    prevButton.style.display = 'flex';
    prevButton.style.opacity = '1';
  }
  
  if (nextButton) {
    nextButton.className = 'carousel-control carousel-control-next absolute top-1/2 right-4 transform -translate-y-1/2 bg-terracotta hover:bg-terracotta-light rounded-full p-2 shadow-lg z-10';
    nextButton.style.display = 'flex';
  ...
  setTimeout(() => {
    ...
    if (carouselInner && totalSlides > 0) {
      ...
      indicators.forEach((indicator, index) => {
        ...
        addEventListenerWithCleanup(carousel, indicator, 'click', handleIndicatorClick);
      });
      ...
      addEventListenerWithCleanup(carousel, carousel, 'mouseenter', handleMouseEnter);
      addEventListenerWithCleanup(carousel, carousel, 'mouseleave', handleMouseLeave);
